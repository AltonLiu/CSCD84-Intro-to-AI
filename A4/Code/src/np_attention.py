...
class MultiHeadAttention:
    ...
    def __call__(self, q, k, v):
        ...
        q_proj = np.dot(q, self.params_q)  # Shape: (batch_size, context_length, embed_dim)
        k_proj = np.dot(k, self.params_k)  # Shape: (batch_size, context_length, embed_dim)
        v_proj = np.dot(v, self.params_v)  # Shape: (batch_size, context_length, embed_dim)

 
        q_proj = q_proj.reshape(batch_size, context_length, self.num_heads, head_dim).transpose(0, 2, 1, 3)
        k_proj = k_proj.reshape(batch_size, context_length, self.num_heads, head_dim).transpose(0, 2, 1, 3)
        v_proj = v_proj.reshape(batch_size, context_length, self.num_heads, head_dim).transpose(0, 2, 1, 3)

        
        attn_scores = np.matmul(q_proj, k_proj.transpose(0, 1, 3, 2)) * scale  # Shape: (batch_size, num_heads, context_length, context_length)
        attn_weights = softmax(attn_scores)  # Shape: (batch_size, num_heads, context_length, context_length)
        attn_output = np.matmul(attn_weights, v_proj)  # Shape: (batch_size, num_heads, context_length, head_dim)
        attn_output = attn_output.transpose(0, 2, 1, 3).reshape(batch_size, context_length, embed_dim)
        out = np.dot(attn_output, self.param_out)  # Shape: (batch_size, context_length, embed_dim)
